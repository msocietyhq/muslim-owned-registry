...
const GUEST_KEY = "mosg_list_quick_guest";
const UPLOAD = "/api/list-quick/upload";

function guestId() {
  if (typeof window === "undefined") return "";
  const current = window.sessionStorage.getItem(GUEST_KEY);
...
export function ListQuickForm() {
  ...
  useEffect(() => {
    if (stage === "form") window.scrollTo({ top: 0, behavior: "smooth" });
  }, [step, stage]);
  ...
  async function confirmCode(event: FormEvent) {
    ...
    setError("");
    try {
      const response = await fetch("/api/list-quick", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      setError(err instanceof Error ? err.message : t.quickList.failVerify);
    } finally {
      setBusy(false);
    }
  }
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (verificationType === "uen_document") {
        ...
        upload.append("guestId", guest);
        const uploaded = await fetch(UPLOAD, { method: "POST", body: upload });
        const data = await uploaded.json();
        if (!uploaded.ok || !data.path) throw new Error(data.error || t.add.fileNeeded);
        uenStoragePath = data.path;
      }
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || t.quickList.failSend);
      setStage("verify");
      setMessage(
        data.devCode
          ? `Dev mode: your code is ${data.devCode}. A confirm link was also created.`
          : t.quickList.sent,
      );
    } catch (err) {
      setError(err instanceof Error ? err.message : t.quickList.failSend);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className={`${ui.shell} ${ui.section} ${ui.legal}`}>
      ...
      <p className="mb-6 leading-relaxed">{t.quickList.lead}</p>
      {foundingOpen ? (
      ...
      {message ? <p className={`${ui.notice} mb-4`}>{message}</p> : null}
      {error ? <p className={`${ui.noticeError} mb-4`}>{error}</p> : null}

      {stage === "verify" ? (
        <form onSubmit={confirmCode} className={`${ui.form} mt-4`}>
          <div>
            <label className={ui.label} htmlFor="listQuickCode">
              {t.quickList.code}
            </label>
            <input
              className={ui.input}
          ...
          <button className={ui.button} type="submit" disabled={busy}>
            {busy ? t.quickList.verifying : t.quickList.verify}
          </button>
        </form>
      ) : (
        <form className={ui.form} onSubmit={onSubmit}>
          ...
          <div className={step === 0 ? "grid gap-4" : "hidden"}>
            {guest ? (
              <AiListingAssist
                ...
                draftPath="/api/list-quick/draft"
                uploadUrl={UPLOAD}
                labels={{
                  open: t.add.aiOpen,
                  close: t.add.aiClose,
            ...
            <div>
              <label className={ui.label} htmlFor="loginEmail">
                {t.quickList.loginEmail}
              </label>
              <p className={`${ui.small} mb-1.5`}>{t.quickList.loginEmailHint}</p>
              <input
                className={ui.input}
                id="loginEmail"
                name="loginEmail"
                ...
                onBlur={() => {
                  if (!contactEmail.trim() && loginEmail.trim()) setContactEmail(loginEmail.trim());
                }}
              />
            </div>
            <div>
          ...
          <ListingWizardNav
            ...
            submitLabel={busy ? t.quickList.submitting : t.quickList.submit}
            onBack={() => setStep((current) => Math.max(0, current - 1))}
            submitDisabled={!allAccepted || !guest}
          />
        </form>
      )}
    </div>
